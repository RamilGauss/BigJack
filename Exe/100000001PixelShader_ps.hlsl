...
struct PS_INPUT
{
			...
			INTERPOLANT( float3 normal, 2 );
				...
				FLAT_INTERPOLANT( float biNormalReflection, 4 );
			...
			INTERPOLANT( float2 uv0, 5 );
					...
					INTERPOLANT( float4 posL3, 9 );
				
			
				
					INTERPOLANT( float4 posL4, 10 );
				
			
		

		INTERPOLANT( float depth, 11 );
	...
	#define OGRE_SAMPLE_SHADOW_ESM( tex, sampler, uv ) tex.SampleLevel( sampler, uv, 0 ).x
		...
		SamplerComparisonState shadowSampler: register(s2);
			...
			Texture2D<float> texShadowMap0				: register(t2);
				...
				#define inPs_posL3 inPs.posL3
			
		
			
				#define inPs_posL4 inPs.posL4
...
)
{
	...
	pixelData.perceptualRoughness = material.kS.w;
		...
		//Get the TBN matrix
		float3 vBinormal	= normalize( cross( pixelData.geomNormal, vTangent ) * inPs.biNormalReflection );
	...
	//Everything's in Camera space
		...
		float spotCosAngle;
			...
			finalColour += BRDF( light0Buf.lights[0].position.xyz, light0Buf.lights[0].diffuse.xyz, light0Buf.lights[0].specular, pixelData ) * fShadow;
	
	
		
			finalColour += BRDF( light0Buf.lights[1].position.xyz, light0Buf.lights[1].diffuse.xyz, light0Buf.lights[1].specular, pixelData )
	...
	;
			...
			finalColour += BRDF( light0Buf.lights[2].position.xyz, light0Buf.lights[2].diffuse.xyz, light0Buf.lights[2].specular, pixelData )
